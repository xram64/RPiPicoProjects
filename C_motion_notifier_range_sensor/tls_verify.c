...
// Root certificate for verifying TLS connections (ISRG Root X2)
// #define TLS_ROOT_CERT "-----BEGIN CERTIFICATE-----\n\
...
int main() {
    ...
    // Verify using root certificate
    const uint8_t root_cert[] = TLS_ROOT_CERT;
    bool pass1 = run_tls_client_test(root_cert, sizeof(root_cert), TLS_CLIENT_SERVER, TLS_CLIENT_HTTP_REQUEST, TLS_CLIENT_TIMEOUT_SECS);
    if (pass1) {
        printf("Test passed\n");
    } else {
        printf("Test failed\n");
